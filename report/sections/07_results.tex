...
\begin{table}[htb]
    ...
    \end{center}
    \caption{Testing \openpose directional invariance against its own data provides the following results for the F-test based on ANOVA\@. The p-values indicate that the $\text{h}_0$ holds for the most parts except in the case East to North, where $p=0.018$ is $p<\alpha$ for a $\alpha=0.05$\@. However the large q-value points to that this low p-value is a false positive.}
